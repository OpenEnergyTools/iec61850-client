import { WebSocketClient, WebSocketServer } from "https://deno.land/x/websocket/lib/websocket.ts";
...
function mmsAssociate(msg:Associate):void {
  ...
  if (connections.has(id))
    ws.send(`IED ${id} already associated!`);
  else { 
...
function mmsRead(msg:Read):void {
  ...
  if (!connections.has(id)) mmsAssociate(msg.conn);
